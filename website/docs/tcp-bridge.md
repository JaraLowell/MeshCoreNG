...
The `Duty used` and `Duty left` tiles show the node's hourly RF transmit duty-cycle budget as timers. `Duty used` is the RF TX time already consumed from the current one-hour budget, and `Duty left` is the RF TX time still available. For example, with `set dutycycle 10`, the hourly RF TX budget is 10% of one hour, or 360 seconds. In that case `Duty used` of `3m 00s` means half of the budget is gone, and `Duty left` shows `3m 00s`. Older bridge firmware that does not send RF duty telemetry will show the tiles as unavailable until the repeater is updated.
