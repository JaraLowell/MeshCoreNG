...
De tegels `Duty used` en `Duty left` tonen het RF TX dutycycle-uurbudget van die node als timers. `Duty used` is de RF TX tijd die al uit het huidige uurbudget is verbruikt, en `Duty left` is de RF TX tijd die nog beschikbaar is. Bij `set dutycycle 10` is het RF TX-budget 10% van een uur, dus 360 seconden. Als `Duty used` dan `3m 00s` toont, is de helft van het budget op en toont `Duty left` ook `3m 00s`. Oudere bridge firmware die deze RF duty-telemetry nog niet meestuurt toont deze tegels als niet beschikbaar tot de repeater is bijgewerkt.
