...
Open die pagina op de server zelf, of vervang `localhost` door het IP-adres van de server vanaf een andere machine op hetzelfde netwerk. De pagina toont online en recent geziene bridge-nodes, firmwareversie, heartbeat-status, packet counters, lokale neighbour count per node wanneer nieuwe firmware die meestuurt, en RF dutycycle-budgetverbruik.
