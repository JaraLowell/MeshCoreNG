...
void MQTTBridge::initializeWiFiInTask() {
  ...
  if (_external_wifi) {
    // WiFi is owned by another subsystem (e.g. TCP bridge). Do not touch the radio;
    // just wait for/observe the existing connection. Slot setup is still deferred to
    // mqttTaskLoop() after NTP sync.
    MQTT_DEBUG_PRINTLN("WiFi managed externally; MQTT bridge will use existing connection");
    return;
  }
...
bool MQTTBridge::handleWiFiConnection(unsigned long now) {
  ...
  if (current_wifi_status == WL_CONNECTED) {
    if (_last_wifi_status != WL_CONNECTED) {
      ...
      #ifdef ESP_PLATFORM
      if (!_external_wifi) {  // WiFi power-save/TX-power owned by the WiFi owner
    ...
    if (_last_wifi_status == WL_CONNECTED) {
      ...
      if (disconnected_duration >= delay_ms && elapsed_since_attempt >= delay_ms) {
        ...
        if (!_external_wifi) {  // another subsystem owns WiFi reconnection
          WiFi.disconnect();
          WiFi.begin(_prefs->wifi_ssid, _prefs->wifi_password);
        }
      }
