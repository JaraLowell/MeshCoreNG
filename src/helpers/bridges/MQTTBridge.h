...
private:
  ...
  // When true, another subsystem (e.g. the TCP bridge) owns the WiFi STA connection.
  // The MQTT bridge then never calls WiFi.begin()/disconnect()/mode() — it only observes
  // WiFi.status() and proceeds when connected. Prevents the two bridges fighting over WiFi.
  bool _external_wifi = false;
...
public:
  ...
  /** When true, WiFi is owned by another subsystem; the MQTT bridge only uses the
   *  existing connection and never (re)connects WiFi itself. Call before begin(). */
  void setExternalWiFiManagement(bool external) { _external_wifi = external; }
